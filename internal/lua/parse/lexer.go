...
func (sc *Scanner) TokenError(tok ast2.Token, msg string) *Error {
...
func (sc *Scanner) scanNumber(ch int, buf *bytes.Buffer) error {
	...
	_ = sc.scanDecimal(ch, buf)
	if sc.Peek() == '.' {
		_ = sc.scanDecimal(sc.Next(), buf)
	}
...
func (sc *Scanner) scanString(quote int, buf *bytes.Buffer) error {
	...
	for ch != quote {
		...
		if ch == '\\' {
			if err := sc.scanEscape(buf); err != nil {
				return err
			}
		} else {
			writeChar(buf, ch)
...
func (sc *Scanner) scanEscape(buf *bytes.Buffer) error {
	ch := sc.Next()
	switch ch {
	case 'a':
		buf.WriteByte('\a')
...
func (sc *Scanner) Scan(lexer *Lexer) (ast2.Token, error) {
redo:
	var err error
	tok := ast2.Token{}
	newline := false
...
func (lx *Lexer) TokenError(tok ast2.Token, message string) {
	panic(lx.scanner.TokenError(tok, message))
}

func Parse(reader io.Reader, name string) (chunk []ast2.Stmt, err error) {
	lexer := &Lexer{NewScanner(reader, name), nil, false, ast2.Token{Str: ""}, TNil}
	chunk = nil
	defer func() {
		if e := recover(); e != nil {
			err, _ = e.(error)
...
func Dump(chunk []ast2.Stmt) string {
	return dump(chunk, 0, "   ")
}
