...
import (
	"fmt"
	ast2 "github.com/Frank-Mayer/yab/internal/lua/ast"
	"math"
	"reflect"
)
...
func sline(pos ast2.PositionHolder) int {
	return pos.Line()
}

func eline(pos ast2.PositionHolder) int {
	line := pos.LastLine()
	if line == 0 {
		return pos.Line()
	}
...
func isVarArgReturnExpr(expr ast2.Expr) bool {
	switch ex := expr.(type) {
	case *ast2.FuncCallExpr:
		return !ex.AdjustRet
	case *ast2.Comma3Expr:
		return !ex.AdjustRet
	}
	return false
}

func lnumberValue(expr ast2.Expr) (LNumber, bool) {
	if ex, ok := expr.(*ast2.NumberExpr); ok {
...
func newCodeBlock(localvars *varNamePool, blabel int, parent *codeBlock, pos ast2.PositionHolder, firstGotoIndex int) *codeBlock {
...
func (fc *funcContext) EnterBlock(blabel int, pos ast2.PositionHolder) {
...
func compileChunk(context *funcContext, chunk []ast2.Stmt, untilFollows bool) { // {{{
	for i, stmt := range chunk {
		lastStmt := true
		for j := i + 1; j < len(chunk); j++ {
			_, ok := chunk[j].(*ast2.LabelStmt)
			if !ok {
				lastStmt = false
				break
			}
...
func compileBlock(context *funcContext, chunk []ast2.Stmt) { // {{{
	if len(chunk) == 0 {
		return
	}
	ph := &ast2.Node{}
...
func compileStmt(context *funcContext, stmt ast2.Stmt, isLastStmt bool) { // {{{
	switch st := stmt.(type) {
	case *ast2.AssignStmt:
		compileAssignStmt(context, st)
	case *ast2.LocalAssignStmt:
		compileLocalAssignStmt(context, st)
	case *ast2.FuncCallStmt:
		compileFuncCallExpr(context, context.RegTop(), st.Expr.(*ast2.FuncCallExpr), ecnone(-1))
	case *ast2.DoBlockStmt:
		context.EnterBlock(labelNoJump, st)
		compileChunk(context, st.Stmts, false)
		context.LeaveBlock()
	case *ast2.WhileStmt:
		compileWhileStmt(context, st)
	case *ast2.RepeatStmt:
		compileRepeatStmt(context, st)
	case *ast2.FuncDefStmt:
		compileFuncDefStmt(context, st)
	case *ast2.ReturnStmt:
		compileReturnStmt(context, st)
	case *ast2.IfStmt:
		compileIfStmt(context, st)
	case *ast2.BreakStmt:
		compileBreakStmt(context, st)
	case *ast2.NumberForStmt:
		compileNumberForStmt(context, st)
	case *ast2.GenericForStmt:
...
func compileAssignStmtLeft(context *funcContext, stmt *ast2.AssignStmt) (int, []*assigncontext) { // {{{
	reg := context.RegTop()
	...
	for _, lhs := range stmt.Lhs {
		...
		case *ast2.AttrGetExpr:
			...
			if _, ok := st.Key.(*ast2.StringExpr); ok {
				ac.keyks = true
			}
			acs = append(acs, ac)
...
func compileAssignStmtRight(context *funcContext, stmt *ast2.AssignStmt, reg int, acs []*assigncontext) (int, []*assigncontext) { // {{{
	...
	for namesassigned < lennames {
		...
		if ec.ctype == ecTable {
			if _, ok := expr.(*ast2.LogicalOpExpr); !ok {
...
func compileAssignStmt(context *funcContext, stmt *ast2.AssignStmt) { // {{{
	code := context.Code
	lennames := len(stmt.Lhs)
	...
	for i := lennames - 1; i >= 0; i-- {
		...
		case ecLocal:
			if acs[i].needmove {
				code.AddABC(OP_MOVE, context.FindLocalVar(ex.(*ast2.IdentExpr).Value), reg, 0, sline(ex))
				reg -= 1
			}
		case ecGlobal:
			code.AddABx(OP_SETGLOBAL, reg, context.ConstIndex(LString(ex.(*ast2.IdentExpr).Value)), sline(ex))
			reg -= 1
		case ecUpvalue:
			code.AddABC(OP_SETUPVAL, reg, context.Upvalues.RegisterUnique(ex.(*ast2.IdentExpr).Value), 0, sline(ex))
			reg -= 1
		case ecTable:
			opcode := OP_SETTABLE
			if acs[i].keyks {
...
func compileRegAssignment(context *funcContext, names []string, exprs []ast2.Expr, reg int, nvars int, line int) { // {{{
	lennames := len(names)
	lenexprs := len(exprs)
	namesassigned := 0
	ec := &expcontext{}
...
func compileLocalAssignStmt(context *funcContext, stmt *ast2.LocalAssignStmt) { // {{{
	reg := context.RegTop()
	if len(stmt.Names) == 1 && len(stmt.Exprs) == 1 {
		if _, ok := stmt.Exprs[0].(*ast2.FunctionExpr); ok {
...
func compileReturnStmt(context *funcContext, stmt *ast2.ReturnStmt) { // {{{
...
func compileIfStmt(context *funcContext, stmt *ast2.IfStmt) { // {{{
...
func compileBranchCondition(context *funcContext, reg int, expr ast2.Expr, thenlabel, elselabel int, hasnextcond bool) { // {{{
	...
	switch ex := expr.(type) {
	case *ast2.FalseExpr, *ast2.NilExpr:
		if !hasnextcond {
			code.AddASbx(OP_JMP, 0, elselabel, sline(expr))
			return
		}
	case *ast2.TrueExpr, *ast2.NumberExpr, *ast2.StringExpr:
		if !hasnextcond {
			return
		}
	case *ast2.UnaryNotOpExpr:
		compileBranchCondition(context, reg, ex.Expr, elselabel, thenlabel, !hasnextcond)
		return
	case *ast2.LogicalOpExpr:
		switch ex.Operator {
		case "and":
...
func compileWhileStmt(context *funcContext, stmt *ast2.WhileStmt) { // {{{
	thenlabel := context.NewLabel()
	elselabel := context.NewLabel()
...
func compileRepeatStmt(context *funcContext, stmt *ast2.RepeatStmt) { // {{{
...
func compileBreakStmt(context *funcContext, stmt *ast2.BreakStmt) { // {{{
...
func compileFuncDefStmt(context *funcContext, stmt *ast2.FuncDefStmt) { // {{{
	if stmt.Name.Func == nil {
		...
		astmt := &ast2.AssignStmt{Lhs: []ast2.Expr{stmt.Name.Func}, Rhs: []ast2.Expr{stmt.Func}}
...
func compileNumberForStmt(context *funcContext, stmt *ast2.NumberForStmt) { // {{{
	code := context.Code
	endlabel := context.NewLabel()
	ec := &expcontext{}
	...
	if stmt.Step == nil {
		stmt.Step = &ast2.NumberExpr{Value: "1"}
...
func compileGenericForStmt(context *funcContext, stmt *ast2.GenericForStmt) { // {{{
	code := context.Code
	endlabel := context.NewLabel()
...
func compileLabelStmt(context *funcContext, stmt *ast2.LabelStmt, isLastStmt bool) { // {{{
...
func compileGotoStmt(context *funcContext, stmt *ast2.GotoStmt) { // {{{
...
func compileExpr(context *funcContext, reg int, expr ast2.Expr, ec *expcontext) int { // {{{
	code := context.Code
	sreg := savereg(ec, reg)
	sused := 1
	if sreg < reg {
	...
	case *ast2.AttrGetExpr:
		...
		opcode := OP_GETTABLE
		if _, ok := ex.Key.(*ast2.StringExpr); ok {
	...
	case *ast2.TableExpr:
		...
		return 1
	case *ast2.ArithmeticOpExpr:
		compileArithmeticOpExpr(context, reg, ex, ec)
		return sused
	case *ast2.StringConcatOpExpr:
		compileStringConcatOpExpr(context, reg, ex, ec)
		return sused
	case *ast2.UnaryMinusOpExpr, *ast2.UnaryNotOpExpr, *ast2.UnaryLenOpExpr:
...
func compileExprWithPropagation(context *funcContext, expr ast2.Expr, reg *int, save *int, propergator func(int, *int, *int, int)) { // {{{
...
func compileExprWithKMVPropagation(context *funcContext, expr ast2.Expr, reg *int, save *int) { // {{{
	compileExprWithPropagation(context, expr, reg, save, context.Code.PropagateKMV)
} // }}}

func compileExprWithMVPropagation(context *funcContext, expr ast2.Expr, reg *int, save *int) { // {{{
	compileExprWithPropagation(context, expr, reg, save, context.Code.PropagateMV)
} // }}}

func constFold(exp ast2.Expr) ast2.Expr { // {{{
...
func compileFunctionExpr(context *funcContext, funcexpr *ast2.FunctionExpr, ec *expcontext) { // {{{
...
func compileTableExpr(context *funcContext, reg int, ex *ast2.TableExpr, ec *expcontext) { // {{{
	code := context.Code
	/*
	...
	*/
	...
	for i, field := range ex.Fields {
		...
		if field.Key == nil {
			...
			opcode := OP_SETTABLE
			if _, ok := field.Key.(*ast2.StringExpr); ok {
				opcode = OP_SETTABLEKS
			}
...
func compileArithmeticOpExpr(context *funcContext, reg int, expr *ast2.ArithmeticOpExpr, ec *expcontext) { // {{{
	exp := constFold(expr)
	...
	expr, _ = exp.(*ast2.ArithmeticOpExpr)
	a := savereg(ec, reg)
	b := reg
...
func compileStringConcatOpExpr(context *funcContext, reg int, expr *ast2.StringConcatOpExpr, ec *expcontext) { // {{{
	code := context.Code
	crange := 1
	for current := expr.Rhs; current != nil; {
		if ex, ok := current.(*ast2.StringConcatOpExpr); ok {
			crange += 1
			current = ex.Rhs
		} else {
			current = nil
...
func compileUnaryOpExpr(context *funcContext, reg int, expr ast2.Expr, ec *expcontext) { // {{{
	opcode := 0
	code := context.Code
	var operandexpr ast2.Expr
	switch ex := expr.(type) {
	case *ast2.UnaryMinusOpExpr:
		exp := constFold(ex)
		...
		ex, _ = exp.(*ast2.UnaryMinusOpExpr)
		operandexpr = ex.Expr
		opcode = OP_UNM
	case *ast2.UnaryNotOpExpr:
		switch ex.Expr.(type) {
...
func compileRelationalOpExprAux(context *funcContext, reg int, expr *ast2.RelationalOpExpr, flip int, label int) { // {{{
	code := context.Code
	b := reg
	compileExprWithKMVPropagation(context, expr.Lhs, &reg, &b)
	c := reg
...
func compileRelationalOpExpr(context *funcContext, reg int, expr *ast2.RelationalOpExpr, ec *expcontext) { // {{{
	a := savereg(ec, reg)
	code := context.Code
...
func compileLogicalOpExpr(context *funcContext, reg int, expr *ast2.LogicalOpExpr, ec *expcontext) { // {{{
	a := savereg(ec, reg)
	code := context.Code
	endlabel := context.NewLabel()
...
func compileLogicalOpExprAux(context *funcContext, reg int, expr ast2.Expr, ec *expcontext, thenlabel, elselabel int, hasnextcond bool, lb *lblabels) { // {{{
	// TODO folding constants?
	code := context.Code
	flip := 0
	jumplabel := elselabel
	...
	case *ast2.TrueExpr:
		...
		return
	case *ast2.NumberExpr, *ast2.StringExpr:
		if thenlabel == lb.e {
			compileExpr(context, reg, expr, ec)
			code.AddASbx(OP_JMP, 0, lb.e, sline(expr))
		} else {
	...
	if ident, ok := expr.(*ast2.IdentExpr); ok && (isLastAnd || isLastOr) && getIdentRefType(context, context, ident) == ecLocal {
...
func compileFuncCallExpr(context *funcContext, reg int, expr *ast2.FuncCallExpr, ec *expcontext) int { // {{{
	funcreg := reg
...
func loadRk(context *funcContext, reg *int, expr ast2.Expr, cnst LValue) int { // {{{
...
func getIdentRefType(context *funcContext, current *funcContext, expr *ast2.IdentExpr) expContextType { // {{{
	if current == nil {
		return ecGlobal
...
func getExprName(context *funcContext, expr ast2.Expr) string { // {{{
	switch ex := expr.(type) {
	case *ast2.IdentExpr:
		return ex.Value
	case *ast2.AttrGetExpr:
...
func Compile(chunk []ast2.Stmt, name string) (proto *FunctionProto, err error) { // {{{
	defer func() {
	...
	err = nil
	parlist := &ast2.ParList{HasVargs: true, Names: []string{}}
	funcexpr := &ast2.FunctionExpr{ParList: parlist, Stmts: chunk}
	if len(chunk) > 0 {
